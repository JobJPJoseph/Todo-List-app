...
class List {
    ...
    removeItem(index) { // logically error with the edge case of input: 0
    ...
    printBoard() {
        ...
        for (let i = 0; i < this.length; i++) {
            console.log([`${i}`.padEnd(5), this.items[i].title.padEnd(13), this.items[i].deadline, this.items[i].check()].join(" | "));
        }
        console.log(str);
        return true;
    }
